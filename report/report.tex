...
\subsubsection{Live stream adaptation of clustering algorithms}
...
There are two test which can trigger model rebuild: full reservoir---its size is defined by user, or positive outcome of \emph{change point detection} test on incoming stream---it detects ``significant'', ``sudden'' change on the input stream.\\
