...
\begin{description}
\item[\textit{D}etect] is about the raw event, for example a cash deposit event.
\item [\textit{D}erive] is about the situation, i.e. "did something happen?", for example there is a suspicious account.
\item [\textit{D}ecide] is about the decision of what to do, for example the decision to determine a risk score or determine another course of action
\item [\textit{D}o] is the action, for example an action that opens an investigation
\end{description}

\subsection{Applications}
in gifh throuptut services\\
possibilities of rules injections

\subsection{Examples}
some example sot motivate the use

\subsubsection{Nuclear plant}
give example of nucleat planr and its controls\\
\subsubsection{Stock market}
asdf\\
\subsubsection{CEP at CERN}
how are we going to use its capabilities at CERN
